...
'              <label>回放<br/>速度</label>' +
'              <input id="speed-input" class="span1 speed" type="text" value="1" />' +
'              <div id="speed-slider"></div>' +
'            </div>' +
'          </li>' +
'          <li class="ctrl">' +
'            <a id="load-tracks-btn" href="#"><i class="icon-map-marker icon-large"></i>上传轨迹</a>' +
'          </li>' +
'        </ul>' +
'      </div>' +
'    </div>' +
...
'    <h3 id="load-tracks-label">加载GPS轨迹数据</h3>' +
'  </div>' +
...
'    <button id="load-tracks-save" class="btn btn-primary">加载</button>' +
'  </div>' +
'</div>',
  ...
  _setup: function() {
    ...
    $('#cursor-date').html(L.Playback.Util.LocalDateStr(startTime));
  ...
  _clockCallback: function(ms) {
      $('#cursor-date').html(L.Playback.Util.LocalDateStr(ms));
