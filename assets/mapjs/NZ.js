...
/**
 * Function: namespace
 * Create a namespace given a string, eg: NZ.namespace("NZ.widget.ui");
 *
 * Parameters:
 ...
 */
NZ.namespace = function (ns, context) {
    ...
    for (var i = 0; i < num; ++i) {
        ...
        if (parts.length > 0) {
            NZ.namespace([parts.join('.')], context[base]);
        }
    }
};
...
/**
 * Constructor: NZ.Class
 * Base class used to construct all other classes. Includes support for 
 *     multiple inheritance. 

 * To create a new NZ-style class, use the following syntax:
 * (code)
 *     var MyClass = NZ.Class(prototype);
 * (end)
 *
 * To create a new NZ-style class with multiple inheritance, use the
 *     following syntax:
 * (code)
 *     var MyClass = NZ.Class(Class1, Class2, prototype);
 * (end)
 * 
 ...
 */
NZ.configure = function (destination, source, getAll) {
 ...
 */
NZ.Util = NZ.Util || {};
NZ.Util.extend = function (destination, source) {
    destination = destination || {};
    if (source) {
        for (var property in source) {
            var value = source[property];
